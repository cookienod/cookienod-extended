...
<div class="wrap cookienod-consent-log">
    ...
    <div class="actions">
        <div class="alignleft">
            <select id="export-format">
                <option value="csv"><?php esc_html_e('CSV', 'cookienod-extended'); ?></option>
            </select>

            <button type="button" class="button" id="export-consent-log">
                <?php esc_html_e('Export Log', 'cookienod-extended'); ?>
            </button>

            <button type="button" class="button button-link-delete" id="clear-consent-log">
                <?php esc_html_e('Clear Log', 'cookienod-extended'); ?>
            </button>
        </div>
                ...
                printf(
                    esc_html(
                        sprintf(
                            ...
                            _n('%s item', '%s items', $cookienod_total_items, 'cookienod-extended'),
    ...
    <?php if ($cookienod_consents) : ?>
        ...
        <table class="wp-list-table widefat striped">
            <thead>
                <tr>
                    <th><?php esc_html_e('ID', 'cookienod-extended'); ?></th>
                    <th><?php esc_html_e('User', 'cookienod-extended'); ?></th>
                    <th><?php esc_html_e('IP Address', 'cookienod-extended'); ?></th>
                    <th><?php esc_html_e('Preferences', 'cookienod-extended'); ?></th>
                    <th><?php esc_html_e('Date', 'cookienod-extended'); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($cookienod_consents as $cookienod_consent) : ?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php else : ?>
                                <?php esc_html_e('Anonymous', 'cookienod-extended'); ?>
                            <?php endif; ?>
                        </td>
                        ...
                        <td>
                            <div class="cookienod-preferences">
                                <?php if (empty($cookienod_display_preferences)) : ?>
                                    <span class="cookienod-pref-badge disabled"><?php esc_html_e('Rejected All', 'cookienod-extended'); ?></span>
                                <?php else : ?>
                                    <?php
                    ...
                    <span class="pagination-links">
                        <?php if ($cookienod_paged > 1) : ?>
                            <a href="<?php echo esc_url(add_query_arg('paged', $cookienod_paged - 1)); ?>" class="prev-page button">
                                <span class="screen-reader-text"><?php esc_html_e('Previous', 'cookienod-extended'); ?></span>
                                ‹
                            </a>
                        <?php endif; ?>
                        ...
                        <?php if ($cookienod_paged < $cookienod_total_pages) : ?>
                            <a href="<?php echo esc_url(add_query_arg('paged', $cookienod_paged + 1)); ?>" class="next-page button">
                                <span class="screen-reader-text"><?php esc_html_e('Next', 'cookienod-extended'); ?></span>
                                ›
                            </a>
                        <?php endif; ?>
                    </span>
    ...
    <?php else : ?>
        ...
        <div class="cookienod-card">
            <div class="cookienod-empty-state">
                <h3><?php esc_html_e('No Consent Records', 'cookienod-extended'); ?></h3>
                <p><?php esc_html_e('Consent records will appear here once users interact with the consent banner on your website.', 'cookienod-extended'); ?></p>
            </div>
        </div>
    ...
    <div class="cookienod-card" style="margin-top: 20px;">
        <h3><?php esc_html_e('About Consent Logging', 'cookienod-extended'); ?></h3>

        <p>
            <?php esc_html_e('Consent logging helps you comply with GDPR, CCPA, and other privacy regulations by keeping a record of user consent choices.', 'cookienod-extended'); ?>
        </p>

        <ul>
            <li><?php esc_html_e('Logs include: User ID (if logged in), IP address, consent preferences, and timestamp', 'cookienod-extended'); ?></li>
            <li><?php esc_html_e('Logs are stored locally in your WordPress database', 'cookienod-extended'); ?></li>
            <li><?php esc_html_e('Export logs for compliance audits', 'cookienod-extended'); ?></li>
            <li><?php esc_html_e('Consider data retention policies when managing logs', 'cookienod-extended'); ?></li>
        </ul>
    </div>
</div>
