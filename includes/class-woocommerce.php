...
class CookieNod_WooCommerce {
    ...
    public function add_marketing_consent_checkbox($checkout) {
        ...
        $label = $this->options['wc_marketing_consent_label'] ?? __('I want to receive marketing emails and offers', 'cookienod-extended');
        ...
        if ($this->options['wc_analytics_consent'] ?? false) {
            $analytics_label = $this->options['wc_analytics_consent_label'] ??
                __('I consent to analytics tracking for order improvement', 'cookienod-extended');
        ...
        $privacy_text = $this->options['wc_privacy_notice'] ?? sprintf(
            ...
            __('Your personal data will be used to process your order. See our %s for details.', 'cookienod-extended'),
            '<a href="' . esc_url(get_privacy_policy_url()) . '">' . esc_html__('Privacy Policy', 'cookienod-extended') . '</a>'
        );
    ...
    public function display_consent_in_admin($order) {
        ...
        echo '<h4>' . esc_html__('Cookie Consent', 'cookienod-extended') . '</h4>';
        echo '<p><strong>' . esc_html__('Marketing:', 'cookienod-extended') . '</strong> ' .
             ($marketing === 'yes' ? '✓ ' . esc_html__('Consented', 'cookienod-extended') : '✗ ' . esc_html__('Not consented', 'cookienod-extended')) .
             '</p>';
        if ($analytics) {
            echo '<p><strong>' . esc_html__('Analytics:', 'cookienod-extended') . '</strong> ' .
                 ($analytics === 'yes' ? '✓ ' . esc_html__('Consented', 'cookienod-extended') : '✗ ' . esc_html__('Not consented', 'cookienod-extended')) .
                 '</p>';
        }
        echo '</div>';
    }
    ...
    public function maybe_show_cart_cookie_notice() {
        ...
        if (!$has_functional_consent && $this->options['wc_show_cookie_notice'] ?? true) {
            wc_print_notice(
                __('Some cart features require functional cookies. Please accept cookies in the banner above for the best shopping experience.', 'cookienod-extended'),
                'notice'
            );
        }
    }
    ...
    public function add_privacy_preferences_to_account() {
        ...
        ?>
        <h3><?php esc_html_e('Privacy Preferences', 'cookienod-extended'); ?></h3>
        <p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
            <label for="cookienod_marketing_consent_account">
                ...
                <?php esc_html_e('I want to receive marketing emails and offers', 'cookienod-extended'); ?>
            </label>
        </p>

        <p>
            <a href="#" onclick="if(window.CookieScanner) { window.CookieScanner.showConsentBanner(); } return false;" class="button">
                <?php esc_html_e('Manage Cookie Preferences', 'cookienod-extended'); ?>
            </a>
        </p>
        <?php
    }
    ...
    public function add_privacy_policy_content() {
        ...
        $content = sprintf(
            ...
            See our %s for more information.', 'cookienod-extended'),
            '<a href="' . esc_url(get_privacy_policy_url()) . '">' . esc_html__('Privacy Policy', 'cookienod-extended') . '</a>'
        );

        wp_add_privacy_policy_content(
            __('CookieNod WooCommerce', 'cookienod-extended'),
    ...
    public static function get_settings_fields() {
        return array(
            'wc_enable_marketing_consent' => array(
                'type' => 'checkbox',
                'label' => __('Enable Marketing Consent Checkbox', 'cookienod-extended'),
                'description' => __('Add a marketing consent checkbox at checkout', 'cookienod-extended'),
                'default' => true,
            ),
            'wc_marketing_consent_label' => array(
                'type' => 'text',
                'label' => __('Marketing Consent Label', 'cookienod-extended'),
                'description' => __('Text shown next to the marketing consent checkbox', 'cookienod-extended'),
                'default' => __('I want to receive marketing emails and offers', 'cookienod-extended'),
            ),
            'wc_marketing_consent_default' => array(
                'type' => 'select',
                'label' => __('Default State', 'cookienod-extended'),
                'options' => array(
                    'no' => __('Unchecked', 'cookienod-extended'),
                    'yes' => __('Checked', 'cookienod-extended'),
                ),
                'default' => 'no',
            ),
            'wc_analytics_consent' => array(
                'type' => 'checkbox',
                'label' => __('Enable Analytics Consent Checkbox', 'cookienod-extended'),
                'description' => __('Add an analytics consent checkbox at checkout', 'cookienod-extended'),
                'default' => false,
            ),
            'wc_show_cookie_notice' => array(
                'type' => 'checkbox',
                'label' => __('Show Cookie Notice on Cart/Checkout', 'cookienod-extended'),
                'description' => __('Display a notice if functional cookies are not accepted', 'cookienod-extended'),
                'default' => true,
            ),
        );
    }
