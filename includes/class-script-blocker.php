...
class CookieNod_Script_Blocker {
    ...
    private function wrap_blocked_script($script, $category) {
        ...
        $placeholder = sprintf(
            ...
            esc_attr($category),
            esc_attr(base64_encode($script)),
            esc_html__('Blocked:', 'cookienod-extended'),
            esc_html($type),
            esc_attr($category),
            esc_html__('Load Script', 'cookienod-extended'),
            $script
        );
    ...
    private function wrap_blocked_iframe($iframe) {
        ...
        $placeholder = sprintf(
            ...
            '</div>',
            esc_attr($src),
            esc_html__('Blocked content from:', 'cookienod-extended'),
            esc_html($host),
            esc_html__('Load Content', 'cookienod-extended')
        );
    ...
    public function output_placeholder_styles() {
        ...
        wp_enqueue_style(
            ...
            COOKIENOD_EXTENDED_PLUGIN_URL . 'assets/css/script-blocker.css',
            array(),
    ...
    private function output_silent_mode_script() {
        wp_register_script('cookienod-silent-mode', false, array(), COOKIENOD_EXTENDED_VERSION, true);
