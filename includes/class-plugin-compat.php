...
class CookieNod_Plugin_Compat {
    ...
    public function enqueue_form_scripts() {
        ...
        wp_register_script('cookienod-form-compat', false, array('jquery'), COOKIENOD_EXTENDED_VERSION, true);
...
(function() {
    ...
    public function add_elementor_consent_controls($element, $section_id, $args) {
        ...
        $element->add_control(
            ...
            array(
                'label' => __('Require Consent', 'cookienod-extended'),
                'type' => \Elementor\Controls_Manager::SWITCHER,
                'description' => __('Hide this widget until user gives consent', 'cookienod-extended'),
                'return_value' => 'yes',
                'default' => '',
        ...
        $element->add_control(
            ...
            array(
                'label' => __('Consent Category', 'cookienod-extended'),
                'type' => \Elementor\Controls_Manager::SELECT,
                'options' => array(
                    'marketing' => __('Marketing', 'cookienod-extended'),
                    'analytics' => __('Analytics', 'cookienod-extended'),
                    'functional' => __('Functional', 'cookienod-extended'),
                ),
                'default' => 'marketing',
                'condition' => array(
    ...
    public function gform_consent_notice($message, $form) {
        return $message . '<div class="gform_consent_notice">' .
               esc_html__('Please accept cookies to submit this form.', 'cookienod-extended') .
               '</div>';
    }
    ...
    public function render_cf7_consent_field($tag) {
        ...
        if (!empty($tag->values) && in_array('show_summary', $tag->values)) {
            ...
            $html .= '<p><strong>' . esc_html__('Your Privacy Settings:', 'cookienod-extended') . '</strong></p>';
    ...
    private function get_consent_placeholder($category) {
        ...
        return sprintf(
            ...
            '</div>',
            esc_attr($category),
            esc_html__('This content requires', 'cookienod-extended'),
            esc_html($category_label),
            esc_html__('consent to display.', 'cookienod-extended'),
            esc_attr($category),
            /* translators: %s: Cookie category name (e.g., Marketing, Analytics) */
            esc_html(sprintf(__('Allow %s', 'cookienod-extended'), $category_label))
        );
    }
    ...
    private function get_category_label($category) {
        $labels = array(
            'necessary' => __('Necessary', 'cookienod-extended'),
            'functional' => __('Functional', 'cookienod-extended'),
            'analytics' => __('Analytics', 'cookienod-extended'),
            'marketing' => __('Marketing', 'cookienod-extended'),
        );
